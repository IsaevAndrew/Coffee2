...
class NewWindow(QWidget):
    ...
    def run(self):
        ...
        #print(self.name, self.combo1, self.combo2, self.taste, int(self.price), int(self.v))
        #print(self.flag)
        #print(self.id)
        if not self.flag:
            cur.execute("""UPDATE coffee
                SET Name = ?, degree = ?, ground = ?, taste = ?, price = ?, volume = ? 
                WHERE ID = ?""", (self.name, self.combo1, self.combo2, self.taste, int(self.price), int(self.v), self.id))
        else:
         #   print(1)
          #  print(self.name, self.combo1, self.combo2, self.taste, int(self.price), int(self.v))
            cur.execute("""INSERT INTO coffee (Name, degree, ground, taste, price, volume) 
                           VALUES (?,?,?,?,?,?) """, (self.name, self.combo1, self.combo2, self.taste, int(self.price), int(self.v)))
